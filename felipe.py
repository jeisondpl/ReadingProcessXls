 ...
 # GUARDAR
    ...
    for directory in listDirectory:
        ...
        try:
            ...
            wb = load_workbook(filename=directory,
            ...
            print("Hoja '{}' No encontrada en el libro de la ruta: '{}'".format(
                'DATOS', directory))
            sinHojaDatos += 1
        con += 1
...
if __name__ == '__main__':
    ...
    def final_list(listDirectory, x): return [
        listDirectory[i:i+x] for i in range(0, len(listDirectory), x)]
    output = final_list(listDirectory, x)

    print(
        f"Total archivos .xlsm :{len(listDirectory)}/ {MAX_WORKERS} procesadores")
